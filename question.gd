...
func _generate_question():
	...
	question_label.text = str(frac[0] + "/" + frac[1] + " x " + frac2[0] + "/" + frac2[1])
	...
	for child in answers.get_children():
		var tex = str(ans_arr_dict[counter][0] + "/" + ans_arr_dict[counter][1])
		child.text = tex
...
func generate_fraction():
	...
	var denom = -1
	while (nom > denom):
		nom = rng.randi_range(0, 9)
		denom = rng.randi_range(0, 9)
	return [nom, denom]
